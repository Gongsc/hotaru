...
- **配置**：设置窗口分为“连接 / 常规 / 关于”三页；后端地址、API Key、刷新间隔、CPU/内存告警阈值、曲线区间、节点名称后显示的内容、节点显示（勾选哪些节点进弹窗列表，支持全选/全部取消）、跟随系统/亮色/暗色主题、自签名证书容忍、开机自启、静默启动（启动时不打开主面板，只驻留托盘）均在修改后自动保存。
