...
- **左键弹窗**：贴着托盘图标弹出的小面板。
  - 顶部是汇总上下行速率曲线，区间可选 5 分钟 / 15 分钟 / 1 小时 / 6 小时，悬停显示该时刻的时间与 ↑↓ 速率。
  - 下方按节点列出在线状态、地区与系统；展开后显示 CPU / 内存 / 硬盘 / 流量（含限额）、运行时长与到期剩余、累计上传下载，以及最近 1 小时的网络质量（延迟与丢包，12 格 × 5 分钟）。
  - 可按住标题栏拖动；点“固定”后失焦不再自动收起。
- **右键菜单**：打开面板、刷新面板、设置…、开机自启、退出。
- **配置**：后端地址、API Key、刷新间隔、CPU/内存告警阈值、曲线区间、跟随系统/亮色/暗色主题、自签名证书容忍、开机自启。
