...
- **托盘实时状态**：
  - 图标为动态圆环仪表，颜色反映整体健康：绿 = 正常，黄 = 超过 CPU/内存告警阈值，红 = 严重超载/全部离线，灰 = 后端不可达；汇总模式下有节点离线时右上角显示红点。
...
- **配置**：后端地址、API Key、刷新间隔、告警阈值、自签名证书容忍、开机自启。

## 数据来源（Komari API）

| 用途 | 接口 |
|---|---|
| 实时快照 | WebSocket `/api/clients`（发送 `get` 拉取），失败时自动回退 `GET /api/nodes` + `GET /api/recent/{uuid}` 轮询 |
